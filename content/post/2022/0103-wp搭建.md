...
看起来很合理是不是！！但是进容器后nano一下就会发现nano用鲜红的字体提醒你`"uploads.ini" is a directory`，好，那也可以，我们`rm -f`一下重新编辑，但！但！但它删不掉！！
...
实际上我猜测容器里的`uploads.ini`目录就是我在`docker-compose.yml`里挂载的`volumes`路径，它之所以是空的，所以它删不掉，而为什么它是空的，那毕竟我也没有创建什么东西在里面嘛……

检查文件大小限制有没有修改好可以通过媒体文件上传限制来检查，点开wordpress后台，媒体-添加新文件，可以看到文件上传限制的提示。
...
我的书摘和随想站：[小球飞槎](https://ink.mantyke.icu/)，欢迎有缘分看到这里的朋友有空的时候也来坐一坐~
