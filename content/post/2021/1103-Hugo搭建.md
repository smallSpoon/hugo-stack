...
搭建流程基于Windows 10环境，静态博客程序为Hugo，使用Vercel进行网站托管及自动部署，博客使用MemE主题。
...
**注意**：如果提示“`Error:This user account is blocked.Contact support@vercel.com for more information.` ”，这是由于Vercel不支持大部分国内邮箱注册，可以发邮件和他们沟通解封，也可以换用Gmail等国外邮箱重新注册一个Github账号再登录。
...
然后配置环境变量：在Vercel上点击左上角的用户名，进入`Dashboard`，点击刚刚建立的Hugo项目，进入后在上方选项卡处找到`Setting`进入设置页面，在左侧点击`Environment Variables`，`NAME`处填写`HUGO_VERSION`，`Value`处填写版本号，例如最新版本为`0.89.0`，最后点击加入
...
> 1. 打开Github仓库，点击右上角绿色的Code，选择Download ZIP，下载主题的压缩包，随后解压（随便解压到哪里），并把解压后的文件夹重命名为`meme`——重命名主要是为了之后写起来方便。
>
