...
分别是`assets\scss\partials\sidebar.scss`的以下几个参数
...
```
    <br/>
另外一个新发现！在`_index.md`指定`Titie`后，会一次性修改全部的分类名称，不需要再逐个修改，换分类名爱好者快乐！
...
修改`assets/scss/partials/layout/aritcle.scss`
