...
域名我之前就有，新加了个子域名，A记录指向VPS IP，套CF之类的东西我也不明白，属于误打误撞突然就能用了，邮箱邮件也没搞定，都不说了。

首先在Contabo控制面板重装VPS（之前没搞定，有残留的怎么也删不掉的数据库，干脆重装），重装时把系统换成Debian10（之前用Ubuntu20.04也总报错，不确定是不是Ubuntu的问题，但据纯之说他用的Debian10没问题，我玄学一下）
